...
async function seedDatabase() {
  ...
  try {
    ...
    // Insert the monitoring data in batches to avoid payload size limits
    const batchSize = 20;
    for (let i = 0; i < simulatedMonitoringData.length; i += batchSize) {
      const batch = simulatedMonitoringData.slice(i, i + batchSize);
