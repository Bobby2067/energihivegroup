/**
 * AlphaESS Battery API Client
 * 
 * Enhanced TypeScript client for interacting with AlphaESS battery systems
...
const API_CONFIG = {
  baseUrl: process.env.ALPHAESS_API_URL || 'https://api.alphaess.com/api',
  key: process.env.ALPHAESS_API_KEY,
  secret: process.env.ALPHAESS_API_SECRET,
...
const logger = {
  debug: (message: string, data?: any) => {
    if (API_CONFIG.debugMode) {
      console.debug(`[AlphaESS] ${message}`, data);
    }
  },
  info: (message: string, data?: any) => {
    console.info(`[AlphaESS] ${message}`, data ? JSON.stringify(data) : '');
  },
  warn: (message: string, error?: any) => {
    console.warn(`[AlphaESS] ${message}`, error);
  },
  error: (message: string, error: any) => {
    console.error(`[AlphaESS] ${message}`, error);
  }
};

/**
 * Authenticate with the AlphaESS API and get an access token
...
async function getAuthToken(): Promise<string> {
  ...
  if (API_CONFIG.enableSimulation) {
    const dummyToken = 'simulated-alphaess-token';
    authTokenCache = {
      token: dummyToken,
  ...
  if (!API_CONFIG.key || !API_CONFIG.secret) {
    throw new Error('AlphaESS API credentials not configured. Please check your environment variables.');
  }

  // Otherwise, authenticate with the real API
  try {
    logger.debug('Authenticating with AlphaESS API');
    ...
    if (!response.ok) {
      ...
      throw new Error(`AlphaESS API authentication failed: ${errorData.message || response.statusText}`);
    }
    ...
    logger.info('Successfully authenticated with AlphaESS API');
    return data.accessToken;
  } catch (error) {
    logger.error('Error authenticating with AlphaESS API:', error);
    throw error instanceof Error 
      ? error 
      : new Error('Unknown error during AlphaESS API authentication');
  }
}
...
export async function getBatteryStatus(serialNumber: string): Promise<BatteryStatus> {
  ...
  try {
    ...
    logger.error(`Error fetching AlphaESS battery status for ${serialNumber}:`, error);
...
export async function getBatteryHistory(
  ...
  try {
    ...
    logger.error(`Error fetching AlphaESS battery history for ${serialNumber}:`, error);
...
export async function getSystemInfo(serialNumber: string): Promise<SystemInfo> {
  ...
  try {
    ...
    logger.error(`Error fetching AlphaESS system info for ${serialNumber}:`, error);
...
export async function updateSystemSettings(
  ...
  try {
    ...
    logger.error(`Error updating AlphaESS settings for ${serialNumber}:`, error);
    throw error instanceof Error 
      ? error 
 ...
 * This uses a polling approach since the AlphaESS API doesn't support WebSockets
 * @param serialNumber The battery system's serial number
...
function parseAlerts(alerts: any[]): BatteryAlert[] {
  ...
  return alerts.map(alert => {
    ...
    // This mapping would depend on the actual structure of AlphaESS alerts
    return {
...
function determineSeverity(alert: any): 'info' | 'warning' | 'error' | 'critical' {
  // This would be implemented based on AlphaESS alert codes and types
  const code = alert.code || alert.alertCode || '';
  const level = alert.level || alert.severity || '';
  
  if (level === 'critical' || /^C\d+/.test(code)) {
    return 'critical';
  } else if (level === 'error' || /^E\d+/.test(code)) {
    return 'error';
  } else if (level === 'warning' || /^W\d+/.test(code)) {
    return 'warning';
  } else {
    return 'info';
  }
...
function applyAustralianHistoryOptimizations(entry: BatteryHistoryEntry): void {
  ...
  (entry as any).australianMarket = {
    ...
    exportIncome,
    importCost,
    netSavings: selfConsumptionSavings + batteryDischargeSavings + exportIncome - importCost,
  };
}
...
function generateSimulatedStatus(serialNumber: string): BatteryStatus {
  ...
  const batteryLevel = 20 + Math.floor((seed % 10) * 8) + Math.floor(Math.random() * 20);
  const solarPower = isDaytime ? Math.floor(solarFactor * seasonalSolarFactor * 5000 * (0.7 + Math.random() * 0.3)) : 0;
  ...
  const loadPower = Math.floor((200 + Math.floor(Math.random() * 2800)) * loadFactor); // 200W-3000W adjusted
  ...
  // But only if battery level allows it (don't discharge below 10%, don't charge above 95%)
  let batteryPower = 0;
  if (surplus > 0 && batteryLevel < 95) {
    batteryPower = Math.min(surplus, 3000); // Max charging rate
  } else if (surplus < 0 && batteryLevel > 10) {
    batteryPower = Math.max(surplus, -3000); // Max discharging rate
  }
  ...
  if (isSummer && hourOfDay >= 14 && hourOfDay <= 16 && Math.random() > 0.9) {
  ...
  return {
    ...
    systemStatus,
    batteryTemperature: 25 + Math.random() * 10, // 25-35°C
    inverterTemperature: 30 + Math.random() * 15, // 30-45°C
    gridVoltage: 230 + Math.random() * 10 - 5, // 225-235V
    gridFrequency: 50 + Math.random() * 0.2 - 0.1, // 49.9-50.1Hz
    alerts,
    isSimulated: true
  };
}
...
function generateSimulatedHistory(
  ...
  for (let timestamp = start.getTime(); timestamp <= end.getTime(); timestamp += intervalMs) {
    ...
    // Energy values for the interval
    const solarGeneration = isDaytime ? solarFactor * seasonalFactor * 2 * (0.7 + Math.random() * 0.3) : 0;
    ...
    const homeConsumption = (0.2 + Math.random() * 0.8) * loadFactor; // 0.2-1.0 kWh per interval, adjusted
    ...
    if (surplus > 0) {
      ...
      energyCharged = Math.min(surplus, 1.5); // Max 1.5 kWh per interval to battery
      gridExport = Math.max(0, surplus - energyCharged);
    } else {
      // Deficit: discharge battery and/or import
      energyDischarged = Math.min(Math.abs(surplus), 1.5); // Max 1.5 kWh from battery
      gridImport = Math.max(0, Math.abs(surplus) - energyDischarged);
    }
    
    result.push({
      timestamp: date.toISOString(),
      batteryLevel: 20 + Math.floor(Math.random() * 60), // Random SOC between 20-80%
      solarGeneration, // kWh
      homeConsumption, // kWh
      energyCharged, // kWh
...
function generateSimulatedSystemInfo(serialNumber: string): SystemInfo {
  ...
  // Australian market specific models and configurations
  const australianModels = [
    'AlphaESS SMILE-B3-10',
    'AlphaESS SMILE-B3-12',
    'AlphaESS SMILE-5',
    'AlphaESS SMILE-T10',
    'AlphaESS STORION-T30'
  ];
  
  const modelIndex = Math.floor(Math.random() * australianModels.length);
  const model = australianModels[modelIndex];
  
  // Capacity based on model
  let capacity: number;
  switch (model) {
    case 'AlphaESS SMILE-B3-10':
      capacity = 10.3;
      break;
    case 'AlphaESS SMILE-B3-12':
      capacity = 12.6;
      break;
    case 'AlphaESS SMILE-5':
      capacity = 5.7;
      break;
    case 'AlphaESS SMILE-T10':
      capacity = 10.1;
      break;
    case 'AlphaESS STORION-T30':
      capacity = 30.0;
      break;
    default:
      capacity = 10.0;
  ...
  return {
    serialNumber,
    model,
    firmwareVersion: `v${Math.floor(Math.random() * 3) + 2}.${Math.floor(Math.random() * 10)}.${Math.floor(Math.random() * 10)}`,
    installationDate: installDate.toISOString().split('T')[0],
    batteryCapacity: capacity,
    nominalPower: (Math.floor(Math.random() * 4) + 3) * 1000, // 3-6kW
    lastMaintenance: maintenanceDate.toISOString().split('T')[0],
    systemType: Math.random() > 0.5 ? 'Hybrid' : 'AC-Coupled',
    isSimulated: true
  };
}
